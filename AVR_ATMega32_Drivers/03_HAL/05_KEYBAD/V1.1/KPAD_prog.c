...
ES_t KPAD_enuGetPressedKey(KPAD_t* Copy_pstrKeyPadConfig, u8* Copy_puin8Value)
{
		...
		for(Local_u8ColsIterator=0;Local_u8ColsIterator<KPD_COLOM_NUM;Local_u8ColsIterator++)
 {
		...
		{
			           Local_enuErorState=DIO_enuGetPintValue(Copy_pstrKeyPadConfig->KPAD_strRows[Local_u8RowsIterator].Row_u8Port,Copy_pstrKeyPadConfig->KPAD_strRows[Local_u8RowsIterator].Row_u8Pin,&Local_u8GetKey);
			if(!Local_u8GetKey)
			{
					_delay_ms(50);
			...
			*Copy_puin8Value=KPAD_Au8PadKeys[Local_u8RowsIterator][Local_u8ColsIterator];
               break;
		   }
		}
		Local_enuErorState=DIO_enuSetPinValue(Copy_pstrKeyPadConfig->KPAD_strCol[Local_u8ColsIterator].Col_u8Port,Copy_pstrKeyPadConfig->KPAD_strCol[Local_u8ColsIterator].Col_u8Pin,DIO_u8HIGH);
 }
